...
const handler = async (m, { conn, usedPrefix }) => {
 ...
 /* await conn.sendMessage(
  ...
  const sessionPath = './SomeraBot/';
  try {
    if (!existsSync(sessionPath)) {
      return await conn.sendMessage(
        m.chat,
        { text: `${lenguajeGB['smsAvisoFG']()} 𝙇𝘼 𝘾𝘼𝙍𝙋𝙀𝙏𝘼 (𝑺𝒐𝒎𝒆𝒓𝒂) 𝙉𝙊 𝙀𝙓𝙄𝙎𝙏𝙀 𝙊 𝙀𝙎𝙏𝘼 𝙑𝘼𝘾𝙄́𝘼.*` },
        { quoted: m }
      );
    }
...
handler.tags = ['owner'];
handler.command = /^(deletesomera|dm|clearallsession|deletegata|borrarchats)$/i;
handler.rowner = true
